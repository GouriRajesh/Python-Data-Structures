class Node:
    def __init__(self, value):
        self.value = value
        self.next = None
        self.prev = None


class DoublyLinkedList:
    def __init__(self, value):
        new_node = Node(value)
        self.head = new_node
        self.length = 1

    def print_list(self):
        output = []
        current_node = self.head
        while current_node is not None:
            output.append(str(current_node.value))
            current_node = current_node.next
        print(" <-> ".join(output))

    def append(self, value):
        new_node = Node(value)
        if self.head is None:
            self.head = new_node
        else:
            temp = self.head
            while temp.next is not None:
                temp = temp.next
            temp.next = new_node
            new_node.prev = temp
        self.length += 1
        return True

    # WRITE SWAP_PAIRS METHOD HERE #

    def swap_pairs(self):

        # Initialize a dummy node to simplify the swapping process.
        dummy_node = Node(0)
        dummy_node.next = self.head
        previous_node = dummy_node

        # Loop through the list as long as there's a pair of nodes to swap.
        while self.head is not None and self.head.next is not None:

            # Identify the first node in the pair to be swapped.
            first_node = self.head

            # Identify the second node in the pair to be swapped
            second_node = self.head.next

            # Connect 'previous_node's 'next' pointer to 'second_node'.
            previous_node.next = second_node

            # Connect 'first_node's 'next' pointer to the node after 'second_node'.
            first_node.next = second_node.next

            # Connect 'second_node' back to 'first_node', effectively swapping them.
            second_node.next = first_node

            # Adjust the 'prev' pointers to maintain the doubly-linked structure.
            second_node.prev = previous_node
            first_node.prev = second_node

            # If there's a node after the current pair, set its 'prev' to point back to 'first_node'.
            if first_node.next is not None:
                first_node.next.prev = first_node

            # Move 'head' to the node after the current pair for the next iteration.
            self.head = first_node.next

            # Update 'previous_node' to 'first_node' for the next pair.
            previous_node = first_node

        # Reset the list's head to start at the node after 'dummy_node'.
        self.head = dummy_node.next

        # Ensure the new head's 'prev' is None, indicating the start of the list.
        if self.head is not None:
            self.head.prev = None
            dummy_node.next = None


my_dll = DoublyLinkedList(1)
my_dll.append(2)
my_dll.append(3)
my_dll.append(4)

print('my_dll before swap_pairs:')
my_dll.print_list()

my_dll.swap_pairs()

print('my_dll after swap_pairs:')
my_dll.print_list()

"""
    EXPECTED OUTPUT:
    ----------------
    my_dll before swap_pairs:
    1 <-> 2 <-> 3 <-> 4
    ------------------------
    my_dll after swap_pairs:
    2 <-> 1 <-> 4 <-> 3

"""